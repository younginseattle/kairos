 ...
 */
async function refetchStubRow(supabaseClient, anthropicApiKey, row, { dryRun, candidateProfile, existingId }) {
  // Looked up by id, not by url. The index matches a role by company+title as
  // well as by link, so the row we need may be filed under a different site's
  // URL entirely — a url lookup here would miss exactly the cross-posted rows
  // the index exists to catch, and report them as unreachable.
  ...
  if (!existing)        return { status: "duplicate", detail: "row disappeared between index load and refetch" };
...
export async function runBulkImport(supabaseClient, anthropicApiKey, rows, opts = {}) {
  ...
  // Load the existing pipeline once, indexed by canonical URL AND by
  // company+title identity. A curated list is exactly where URL-only dedup
  // fails: the rows are collected by hand from LinkedIn, an aggregator or a
  // Google result, so the same role routinely arrives under a link that
  // shares nothing with the ATS URL already in the table.
  ...
  for (const row of rows) {
    ...
    // Same identity-aware duplicate check as every other source. Matching on
    // the raw row rather than the normalized job is deliberate: it settles the
    // question before the JD fetch below, so a role already in the pipeline
    // costs no network round-trip.
    const duplicate = jobIndex.find({ ...row, source: row.source || source });
    if (duplicate) {
      const via = duplicate.reason === "identity" ? "already in pipeline from another site" : "same listing";
      if (!refetchStubs) {
        log(`  · Duplicate (${via}) — skipping: "${row.title}" at ${row.company}`);
        // A hand-curated row outranks an aggregator link, so repoint the
        // existing row at it rather than leaving the worse URL in place.
        const upgraded = dryRun ? false : await upgradeExistingJob(
          supabaseClient, duplicate.row,
          { ...row, source: row.source || source }, { log, logError },
        );
        skipped++;
        record(row, "duplicate", upgraded ? `${via} — link upgraded to ${row.source || source}` : via);
        continue;
      }
      const outcome = await refetchStubRow(supabaseClient, anthropicApiKey, row, {
        dryRun, candidateProfile, existingId: duplicate.row.id,
      });
    ...
    if (dryRun) {
      imported++;
      // Index the pretend insert too, so a dry run's counts match what the
      // real run would do rather than reporting the same role twice.
      jobIndex.add(job);
    ...
    try {
      ...
      imported++;
      // Index it immediately: a curated sheet can list the same role twice
      // under two links, and without this the second copy sees an index that
      // predates the first insert and adds a duplicate row.
